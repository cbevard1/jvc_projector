...
import asyncio
from .jvccommands import Commands, Responses, PACK
import time
...
class JVCProjectorClient:
    ...
    def __init__(
        ...
        self._connect_timeout_seconds: int = (
            connect_timeout_seconds if connect_timeout_seconds else 10
        )
    ...
    async def _async_send_command(self, operation: bytes) -> bytes:
        ...
        try:
            ...
            raise JVCCannotConnectError(
                "Could not connect to the projector. Check the Hostname/IP and ensure that 'Control4' is turned off in the network settings."
            )
        ...
        if await reader.read(len(JVC_GREETING)) != JVC_GREETING:
            raise JVCHandshakeError(
                "Projector did not reply with correct PJ_OK greeting."
            )
        ...
        try:
            if not await self.async_is_lamp_on():
                raise JVCPoweredOffError(
                    "Can't send this command, the projector is not powered on."
                )
    ...
    async def async_get_input(self) -> dict[str, str]:
        ...
        if not await self.async_is_lamp_on():
            raise JVCPoweredOffError(
                "Can't get input, the projector is powered off or starting up."
            )
